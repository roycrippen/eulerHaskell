...
getChunks n xs | length xs < n = []
               | otherwise     = chunk : getChunks n (tail xs)
  where
    chunk = if '0' `elem` chunk' then "" else chunk' where chunk' = take n xs
...
p011 = do
    ...
    let grid = fillGrid inputStr
        res  = maximum
            [ prodMax $ direction grid
            | direction <- [east, south, southEast, southWest]
            ]
...
maxCollatz n = runST $ do
    ...
    let collatz x
          ...
          where
            go x | even x    = fmap (+ 1) (collatz (x `div` 2))
...
p016 = do
    let res =
            foldl' (+) 0 (parMapChunked rseq 1000 digitToInt $ show (2 ^ 1000))
...
numToWords n
    ...
    | n < 1000 = numUnits !! (n `div` 100) ++ numAnds n ++ numToWords
        (n `mod` 100)
    | n < 10000 = numUnits !! (n `div` 1000) ++ " thousand " ++ numToWords
        (n `mod` 1000)
    | otherwise = ""
  where
    numUnits =
...
go :: [[Int]] -> [[Int]]
go xss@(xs : ys : zs) = case length xss of
    0 -> []
    1 -> [head xss]
    2 -> [sumTwoRows xs ys]
...
-- days in each month
days y | y `elem` leap = [31, 29, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31]
...
lastDayOfMonth seed mDays | mDays + seed > 7 = lastDayOfMonth seed (mDays - 7)
